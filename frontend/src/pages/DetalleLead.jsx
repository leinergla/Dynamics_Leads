...
export default function DetalleLead() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/leads" className="text-sm font-medium text-indigo-600 hover:text-indigo-800">
          ← Volver a la lista
        </Link>
      </div>
      ...
      {!loading && !error && lead && (
        <>
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900">Detalle del Lead</h1>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 break-all font-mono text-sm text-slate-700">{lead.leadId}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Fecha de creación</p>
              <p className="mt-1 text-sm text-slate-700">{formatFecha(lead.fechaCreacion)}</p>
            </div>
          </div>
            ...
            <dl className="divide-y divide-slate-100">
              {camposDinamicos(lead).map(([k, v]) => (
                <div key={k} className="grid grid-cols-3 gap-4 px-5 py-3">
                  ...
                  <dd className="col-span-2 text-sm text-slate-800">{String(v)}</dd>
                </div>
              ))}
              {camposDinamicos(lead).length === 0 && (
                <p className="px-5 py-4 text-sm text-slate-400">Este lead no tiene campos.</p>
              )}
            </dl>
          </section>
          ...
          <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            {archivos.length === 0 ? (
              <p className="px-5 py-4 text-sm text-slate-400">Este lead no tiene archivos.</p>
            ) : (
              <ul className="divide-y divide-slate-100">
                {archivos.map((a) => (
                  <li key={a.id} className="flex items-center justify-between gap-4 px-5 py-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-slate-800">{a.nombreArchivo}</p>
